...
以下のサンプルコードでは、変数 `a` を0で初期化して、括弧内で `a` に100を代入し、括弧自体は `a + 20` である120を返します。括弧の戻り値は3と和を取られ、結果、プログラムの出力は123になります。
...
複文は、0個以上の文と戻り値となる末尾式を `;` で区切って繋いだものです。
