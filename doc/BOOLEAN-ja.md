...
値の種類によってどのように論理値化が行われるかが異なります。

全般的に、ある値の種類において、最も単純な値が `FALSE` で、それ以外は `TRUE` です。

| 値の種類   | 真 `TRUE` になるもの                | 偽 `FALSE` になるもの             |
|--------|-------------------------------|-----------------------------|
| NULL   | なし                            | `NULL`                      |
| 数値     | 0に等しくない値                      | 0に等しい値                      |
| 論理値    | `TRUE`                        | `FALSE`                     |
...
エルビス演算子 `value ?: default` は、 `value` が `NULL` である場合に `default` を返す演算子です。
