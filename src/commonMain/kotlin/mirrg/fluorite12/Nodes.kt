...
class RegexNode(val left: TokenMatch, val node: LiteralStringContent, val right: TokenMatch, val flags: IdentifierNode?) : Node()
...
class ThrowNode(val operator: List<TokenMatch>, val right: Node) : Node()
class ReturnNode(val left: Node, val operator: List<TokenMatch>, val right: Node) : Node()
...
class InfixExclamationIdentifierNode(val left: Node, val prefix: List<TokenMatch>, val infix: IdentifierNode, val right: Node) : Node()
