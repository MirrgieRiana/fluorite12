...
class Fluorite12Grammar : Grammar<Node>() {
    ...
    val add: Parser<Node> by leftAssociative(mul, -s * (+plus or +(minus * -NotParser(greater)) or +ampersand) * -b, ::infixNode)
    ...
    val stream: Parser<Node> by leftAssociative(assignation, -s * (+(question * pipe) or +(exclamation * pipe) or +pipe) * -b, ::infixNode)
